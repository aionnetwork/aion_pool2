...
bool verifyEH210(const char *hdr, const char *soln, int n, int k){

    blake2b_state state;
    EhInitialiseState(n, k, &state);

    //Update header and nonce in same call
    blake2b_update210(&state, (unsigned char*)&hdr[0], 64);
                  ...
                  unsigned char* hash, size_t hLen)
{
    ...
    blake2b_update210(&state, (const unsigned char*) &lei,
                                      sizeof(eh_index));
    blake2b_final210(&state, hash, hLen);
}

void ExpandArray210(const unsigned char* in, size_t in_len,
...
void CompressArray210(const unsigned char* in, size_t in_len,
                   unsigned char* out, size_t out_len,
                   size_t bit_len, size_t byte_pad)
{
...
// comparison
void EhIndexToArray210(const eh_index i, unsigned char* array)
{
...
// Big-endian so that lexicographic array comparison is equivalent to integer
// comparison
eh_index ArrayToEhIndex210(const unsigned char* array)
{
    eh_index bei;
...
eh_trunc TruncateIndex210(const eh_index i, const unsigned int ilen)
{
    // Truncate to 8 bits
    return (i >> (ilen - 8)) & 0xff;
}

eh_index UnTruncateIndex210(const eh_trunc t, const eh_index r, const unsigned int ilen)
{
    eh_index i{t};
    return (i << (ilen - 8)) | r;
}

std::vector<eh_index> GetIndicesFromMinimal210(std::vector<unsigned char> minimal,
                                            size_t cBitLen)
{
    size_t lenIndices { 8*sizeof(eh_index)*minimal.size()/(cBitLen+1) };
    size_t bytePad { sizeof(eh_index) - ((cBitLen+1)+7)/8 };
    std::vector<unsigned char> array(lenIndices);
    ExpandArray210(minimal.data(), minimal.size(),
                array.data(), lenIndices, cBitLen+1, bytePad);
    std::vector<eh_index> ret;
    for (int i = 0; i < lenIndices; i += sizeof(eh_index)) {
        ret.push_back(ArrayToEhIndex210(array.data()+i));
    }
    return ret;
}

std::vector<unsigned char> GetMinimalFromIndices210(std::vector<eh_index> indices,
                                                 size_t cBitLen)
{
    ...
    for (int i = 0; i < indices.size(); i++) {
        EhIndexToArray210(indices[i], array.data()+(i*sizeof(eh_index)));
    }
    std::vector<unsigned char> ret(minLen);
    CompressArray210(array.data(), lenIndices,
                        ...
                        size_t hLen, size_t cBitLen)
{
    ...
    ExpandArray210(hashIn, hInLen, hash, hLen, cBitLen);
}
                                                          ...
                                                          size_t cBitLen) const
{
    ...
    std::vector<unsigned char> ret(minLen);
    ret.assign(minLen, 0);
    CompressArray210(hash+len, lenIndices, ret.data(), minLen, cBitLen+1, bytePad);
    return ret;
}
...
TruncatedStepRow<WIDTH>::TruncatedStepRow(const unsigned char* hashIn, size_t hInLen,
        ...
        StepRow<WIDTH> {hashIn, hInLen, hLen, cBitLen}
{
    hash[hLen] = TruncateIndex210(i, ilen);
}
...
bool Equihash<N,K>::IsValidSolution(blake2b_state *base_state, std::vector<unsigned char> soln)
{
    ...
    for (eh_index i : GetIndicesFromMinimal210(soln, CollisionBitLength)) {
