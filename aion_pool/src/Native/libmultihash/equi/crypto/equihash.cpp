...
const char* zcash_pow = "ZcashPoW";
const char* aion_pow = "AION0PoW";
EhSolverCancelledException solver_cancelled;

template<unsigned int N, unsigned int K>
int Equihash<N,K>::InitialiseState(eh_HashState& base_state)
{
    return Equihash<N,K>::InitialiseState(base_state, zcash_pow);
}

template<unsigned int N, unsigned int K>
int Equihash<N,K>::InitialiseState(eh_HashState& base_state, const char* personal)
{
    uint32_t le_N = htole32(N);
    uint32_t le_K = htole32(K);
    unsigned char personalization[crypto_generichash_blake2b_PERSONALBYTES] = {};
    memcpy(personalization, personal, 8);
                               ...
                               const std::function<bool(EhSolverCancelCheck)> cancelled)
{
    ...
    for (eh_index g = 0; X.size() < init_size; g++) {
        ...
        for (eh_index i = 0; i < IndicesPerHashOutput && X.size() < init_size; i++) {
            X.emplace_back(tmpHash+(i*HashLen), HashLen, HashLength,
    ...
    int invalidCount = 0;
    {
        ...
        for (eh_index g = 0; Xt.size() < init_size; g++) {
            ...
            for (eh_index i = 0; i < IndicesPerHashOutput && Xt.size() < init_size; i++) {
                Xt.emplace_back(tmpHash+(i*HashLen), HashLen, HashLength, CollisionBitLength,
    ...
    for (std::shared_ptr<eh_trunc> partialSoln : partialSolns) {
        ...
        for (eh_index i = 0; i < soln_size; i++) {
            ...
            for (eh_index j = 0; j < recreate_size; j++) {
                ...
                icv.emplace_back(tmpHash+((newIndex % IndicesPerHashOutput) * HashLen),
                                 HashLen, HashLength, CollisionBitLength, newIndex);
...
template int Equihash<210,9>::InitialiseState(eh_HashState& base_state, const char* personal);
#ifdef ENABLE_MINING
